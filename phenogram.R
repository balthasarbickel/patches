...
# small patch by Balthasar Bickel 2018
...
phenogram<-function(tree,x,fsize=1.0,ftype="reg",colors=NULL,axes=list(),add=FALSE,...){
	...
	if(hasArg(x.axis.intervals)) x.axis.intervals<-list(...)$x.axis.intervals
	else x.axis.intervals<-NULL      
	...
	if(!add){
        if(is.null(x.axis.intervals)) {
		at<-round(0:(nticks-1)*max(H)/(nticks-1),digits)
        axis(1,at=at, labels=at)
        } else {
		axis(1,at=c(max(H)-x.axis.intervals), labels=x.axis.intervals)
        }
        axis(2); title(xlab=xlab,ylab=ylab,main=main,sub=sub)
	}
